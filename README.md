...
**火狐用户的解决方案**：
由于火狐最新机制可能不显示任何可供放行的“小锁”图标，**强烈建议使用 Chrome 或 Edge 浏览器**。

**硬核后门 (如果您非要用火狐，且风险自负)**：
在火狐地址栏输入 `about:config`，搜索 `security.mixed_content.block_active_content` 并将其设置为 `false`。
*(警告：这会导致您的火狐对所有网站失去混合内容保护，极大降低安全性，请谨慎操作！)*
